...
function ConfigInfo()
{
	...
	this.getCardsOfType			= getCardsOfType_M;
	//for page local use, without frames
	this.onCardSelectedLocal		= onCardSelected_MLocal;
	this.getSelectCardHTMLLocal		= getSelectCardHTML_MLocal;
	//for get the LCR/ANALog/QBRI channel right word
	this.getChannelText             = getChannlText_M;
	
    this.m_str1stName = "";
	this.m_str2ndName = "";
    function getChannlText_M(arAddrChannel)
	{
		var strLeg =  this.addr2Name( arAddrChannel[ 0 ] );
		strLeg += " Chan. ";
		strLeg += arAddrChannel[ 1 ];
		
		if ( strLeg.indexOf('LCR') == 0 )
		{
			strLeg = "LCR Resource " + arAddrChannel[ 1 ];		
		}
		if ( strLeg.indexOf('AN4') == 0 )
		{
			strLeg  = this.addr2Name( arAddrChannel[ 0 ] );
			strLeg += ( (arAddrChannel[ 1 ]<=4) ? " FXO " : " FXS " ) ;//+ arAddrChannel[ 1 ];			
			strLeg += ( (arAddrChannel[ 1 ]<=4) ? arAddrChannel[ 1 ] : arAddrChannel[ 1 ]-4 );
		} 
		if ( strLeg.indexOf('QBRI') == 0 )
		{
			strLeg  = this.addr2Name( arAddrChannel[ 0 ] );
			strLeg += "Port " + Math.ceil(arAddrChannel[ 1 ] / 2) + " Ch. " + (((arAddrChannel[ 1 ] - 1) % 2) + 1);
		} 
		return strLeg;
	}
    ...
    function processConfigInfo_M( packStr )
    {
        ...
        for (i=0; i<arCardsAddrs.length; i++)
        {
            ...
            for (var j=0; j<arAddresses.length; j++)
            {
                ...
                var strFullCardName = strCardType;
                if ( nSlot != -1 && strCardType!= "LCR" )
                {
                    // A physical card
                    strFullCardName += "(" + nSlot + ")";
                }
    ...
    function calcSlotNumber_M( strCardAddr )
    {
        switch ( strCardAddr )
        {
            case "MG":  // media gateway
            case "LCR": // LCR              
            {
            	strCardAddr = "2B";
            	break;
            }
            case "DR":  // dynamic routing
            {
                // all the above are not physical entities
                // do not display a slot number.
                return -1;
            }
        }
    ...
    function getCardsOfType_M( packStr, strCardTypes )
	{
		...
		for (var i=0; i<arTypes.length; i++)
		{
			...
			for (var j=0; j<arTypeAddresses.length; j++)
			{
				...
				card.m_nSlot        = this.calcSlotNumber( arTypeAddresses[ j ] );
				arCards.push( card );
			}
		}
    ...
    function getSelectCardHTML_M( packStr, strCardTypes, strTitle, strCallback )
    {
        ...
        var strHTML = "<font face='Arial' id='selSlotTitle'>";
        strHTML += strTitle;
        strHTML += "<br><table id='selSlotTbl' bgcolor='#f0f0f0'><tr><td><b>Slot </b></td>";
        
        var nLastDeck = 0;
        for (var i=0; i<arCards.length; i++ )
        {
            var strCardAddr = arCards[ i ].m_strAddress;
            var strCardName = this.addr2Name( strCardAddr );
            var nSlot = this.calcSlotNumber( strCardAddr );
            ...
            strHTML += "value='" + strCardAddr + "' ";
            if ( strCardAddr == strCheckedCard )
            {
                strHTML += "checked ";
            }
			
			var strCardType	= arCards[ i ].m_strType;
            strHTML += "onClick='G_configInfo.onCardSelected( \"" + strCardGroupType + "\",\"" + strCardAddr + "\"); " + strCallback + "(\"" + strCardAddr + "\", \"" + strCardType + "\");'>" + nSlot;
            strHTML += "</td>";
	...
	function onCardSelected_M( strCardGroupType, strCard )
	{
	 ...
	 function getSelectCardHTML_MLocal( packStr, strCardTypes, strTitle, strCallback )
    {
		// Update the internal data structures (what cards are available, what types,
		// who is connected to who etc.) from the configuration info packet.
		...
		if (typeof G_map[ strCardGroupType ] == 'undefined')
		{
			G_map[ strCardGroupType ] = arCards[ 0 ].m_strAddress;
		}

		var strCheckedCard = G_map[ strCardGroupType ];
		var strSelCardType = "";
		
		
        var strHTML = "<table class='TableNoBorder'><tr><td valign='middle'><font class='BlueAlertBold'>";
        strHTML += strTitle;
        strHTML += ":</font></td><td valign='top'><table class='table'><tr><td class='BlueAlertBold'>Slot</td>";
        ...
        for (var i=0; i<arCards.length; i++ )
        {
            ...
            strHTML += "<td width=20 class='BlueAlertBold'>";
            strHTML += "<input  type=radio name='c' ";
            strHTML += "value='" + strCardAddr + "' ";
            if ( strCardAddr == strCheckedCard )
            {
			...
			var strCardType	= arCards[ i ].m_strType;
            strHTML += "onClick='G_configInfo.onCardSelectedLocal( \"" + strCardGroupType + "\",\"" + strCardAddr + "\"); " + strCallback + "(\"" + strCardAddr + "\", \"" + strCardType + "\");'>" + nSlot;
            strHTML += "</td>";
	...
	function onCardSelected_MLocal( strCardGroupType, strCard )
	{
		G_map[ strCardGroupType ] = strCard;
	}
}
